...
pipeline {
  ...
  parameters {
    ...
    string( name:           'ASYNCAPI_FILE',
            defaultValue:   'asyncapi/asyncapi-sample-jenkins.yaml',
            description:    'The location of the asyncapi file in the repository' )
  }
  environment {
    BUILD_DIR = "__BUILD_DIR__/"
    ASYNCAPI_FILE = "${BUILD_DIR}${params.ASYNCAPI_FILE}"
  }
  stages {
    stage( 'Checkout' ) {
        steps {
    ...
    stage ( 'Extract asyncapi' ) {
      steps {
        script {
          sh "mkdir -p tmp"
          sh "java -jar /home/jenkins/exec-jar/asyncapi-cicd-parser-0.1.0.jar --asyncapi-in=${ASYNCAPI_FILE} --output=tmp/cicd-extract.yaml"
        }
      }
    }
